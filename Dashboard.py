...
# vendedor_vendas = dados.groupby('Vendedor')[['Produto']].count().sort_values('Produto', ascending=False)

# vendedor_receita = dados.groupby('Vendedor')[['Preço']].sum().sort_values('Preço', ascending=False)
...
# fig_vendedor_vendas = px.bar(vendedor_vendas,
#                              text_auto=True,
#                              title='Vendas por vendedor')

# fig_vendedor_receita = px.bar(vendedor_receita,
#                               text_auto=True,
...
with aba3:
    ...
    with coluna1:
        st.metric('Receita',formata_numero(dados['Preço'].sum(), 'R$'))
        # st.plotly_chart(fig_vendedor_vendas, use_container_width=True)
    ...
    with coluna2:
        st.metric('Quantidade de vendas', formata_numero(dados.shape[0]))
        # st.plotly_chart(fig_vendedor_receita,use_container_width=True)
